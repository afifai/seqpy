...
def aligner(object _seqj, object _seqi, \
            ...
            method="global", matrix="BLOSUM62", fmt='alignment'):
    """
    ...
    print("initializing memory...", file=sys.stderr)
    # START HERE:
    #cdef int imethod = {"global": 0, "local": 1, "glocal": 2, "global_cfe": 3}[method]
    if imethod == 0:
        pointer[0, 1:] = LEFT
    ...
    print("filling DP matrices...", file=sys.stderr)
    ...
    print("finding optimal alignment...", file=sys.stderr)
    if method == 'local':
        # max anywhere
        i, j = max_index(score)
    elif method == 'glocal':
    ...
    print("preparing alignment...", file=sys.stderr)
    seqlen = max_i + max_j
